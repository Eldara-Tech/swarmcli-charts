...
- **One service per peer**, each `replicas: 1`, generated by a `range` over a peer
  ...
  1-based N, and that name is reused as the peer's network alias, its identity in
  the clustering protocol, and the address it advertises to the others — one
  string, so a rename cannot desynchronise them.
...
- **`endpoint_mode: dnsrr`**, so a peer's alias resolves to the task's own address.
  A clustering protocol needs the real address, not the load-balanced VIP that
  the default `vip` mode puts in front of it.
- **Publish nothing the cluster uses internally.** Peers on a shared overlay reach
  ...
  — `docker compose config` accepts an ingress port beside `dnsrr` and only the
  daemon rejects the pair.

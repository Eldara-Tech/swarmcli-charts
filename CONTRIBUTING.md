...
**Clustered stateful charts** — a chart whose peers replicate to each other
(`charts/mariadb-galera` is the reference). The single-replica rule above still
holds *per peer*; what changes is that there are now several of them, and the
temptation is to model a cluster as one service with N replicas. Do not: every
replica of a Swarm service shares one spec, so they cannot have distinct volumes,
distinct node pins or distinct identities, which is exactly what a replicating
peer needs.

- **One service per peer**, each `replicas: 1`, generated by a `range` over a peer
  count so the count stays a value. Peer services are named `<chart>-<N>` with a
  1-based N, and that name is reused as the peer's network alias and its identity
  in the clustering protocol — one string, so a rename cannot desynchronise them.
  (What it advertises as its *address* is an IP, not this name; see below.)
- **One volume and one node label per peer**: `<volumePrefix>-<N>` pinned by
  `node.labels.<nodeLabelPrefix>-<N> == true`. Like the single-replica pin it
  travels with `persistence.enabled`, and `nodeLabelPrefix: ""` drops every pin
  for a single-node swarm. Document that peers belong on *different* nodes; three
  peers on one node share its fate and provide no availability.
- **Keep `endpoint_mode: vip` and address peers by `tasks.<release>_<peer>`.** A
  clustering protocol needs each peer's real address, not a load-balanced VIP, and
  `tasks.<service>` is what Swarm publishes for that — a stack's service name being
  `<release>_<service>`. **Do not switch to `endpoint_mode: dnsrr` to get the plain
  service name to resolve.** It looks like the tidier answer and it silently
  disables network aliases, so both the per-peer alias and the shared client alias
  stop existing; `charts/mariadb-galera` shipped that to CI and every state
  transfer failed with `resolve: Host not found (authoritative)`.
- **A peer must ADVERTISE a real IP, not a name.** Read it from `/etc/hosts`, which
  Docker writes for the container's hostname and which needs no DNS, and refuse to
  start if it cannot be established. Anything that has to *bind* a listener to the
  advertised address — Galera's IST receiver, and its equivalents elsewhere — fails
  on a name that does not resolve inside the container, and it fails far from the
  cause. A silent fallback to the name is worse than crashing: it turns a clear
  startup error into a replication error three layers away.
- **Publish nothing the cluster uses internally.** Peers on a shared overlay reach
  each other on every port, so replication ports stay off the host entirely. A
  client port can only be published in `mode: host`: every peer publishes the same
  number, and several services cannot each claim it on the ingress mesh. Guard it
  — `docker compose config` validates structure, not Swarm semantics, so it
  accepts combinations the daemon refuses.
...
- **Assert the cluster in `ci/render-check.sh`.** Convergence is a weak signal —
  ...
  list names every peer, and that exactly as many peers bootstrap as intended.
  The e2e check should assert the cluster's own size and a cross-peer write.
- **A healthcheck is an ACTION, not an observation.** Swarm kills what fails one,
  so a check that asserts a state the software leaves *normally* converts a routine
  operation into a restart. `charts/mariadb-galera` asserted `Synced` because it
  read as strictly better than "is listening" — but a peer donating a state
  transfer is deliberately `Donor/Desynced` for the whole transfer while still
  serving, so once a transfer outran `interval x retries` Swarm killed the donor
  mid-transfer, during exactly the recovery the cluster exists for. e2e missed it
  four times because the test dataset transferred in under a second. Ask which
  states the thing passes through in normal operation, and check "can it serve",
  not "is it in the best possible state".
- **CI's swarm is a single node**, so a per-peer pin cannot be satisfied there
  unless `ci/e2e-setup.sh` labels that one node with every peer's label. Do that
  rather than testing only the unpinned shape: the pinned render is the one
  operators run.
